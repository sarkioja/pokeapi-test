...
export class TeamController {
  ...
  @ApiOperation({
    ...
    description:
      'Creates a new team in `active` status linked to the given trainer. A trainer can have multiple teams.',
  })
  @ApiBody({
    type: CreateTeamDto,
    examples: {
  ...
  @ApiResponse({
    status: 201,
    description: 'Team created with empty Pokémon roster',
    schema: { example: { ...TEAM_EXAMPLE, pokemon: [] } },
  })
  @ApiResponse({
    status: 400,
    description: 'Validation error',
    schema: {
      example: { statusCode: 400, error: 'BAD_REQUEST', message: 'trainerId must be a UUID' },
    },
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiOperation({
    ...
    description:
      'Returns a paginated list of all active (non-deleted) teams belonging to the trainer, ordered by creation date.',
  })
  @ApiParam({
    name: 'trainerId',
  ...
  @ApiResponse({
    ...
    schema: { example: { data: [TEAM_EXAMPLE], total: 1 } },
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns the team with its full Pokémon roster (up to 5), including slot, nickname, types, and base stats for each Pokémon.',
  })
  @ApiParam({
    name: 'id',
  ...
  @ApiResponse({
    status: 200,
    description: 'Team found with Pokémon roster',
    schema: { example: TEAM_EXAMPLE },
  })
  @ApiResponse({
    status: 404,
    ...
    schema: {
      example: {
        statusCode: 404,
        error: 'NOT_FOUND',
        message: 'Team b2c3d4e5-f6a7-8901-bcde-f12345678901 not found',
      },
    },
  })
  ...
  @ApiOperation({
    summary: 'Update team',
    description:
      'Updates the team name and/or status. Setting `status` to `archived` prevents adding new Pokémon to the team. Archived teams can be unarchived by setting `status` back to `active`.',
  })
  @ApiParam({
    name: 'id',
    description: 'Team UUID',
  ...
  @ApiResponse({
    ...
    schema: { example: { ...TEAM_EXAMPLE, name: 'Elite Four Squad' } },
  })
  @ApiResponse({
    status: 400,
  ...
  @ApiOperation({
    ...
    description:
      'Soft-deletes the team. The Pokémon slots are preserved in the database for audit purposes but the team is no longer accessible via the API.',
  })
  @ApiParam({
    name: 'id',
    description: 'Team UUID',
  ...
  @ApiOperation({
    ...
    description:
      'Adds a Pokémon to the team by name. If the Pokémon is not in the local cache (or cache is stale, TTL 24h), it is fetched from PokéAPI and persisted. Business rules: max 5 Pokémon per team, no duplicates, team must not be archived.',
  })
  @ApiParam({
    name: 'id',
    description: 'Team UUID',
  ...
  @ApiResponse({
    status: 201,
    description: 'Pokémon added to team',
    schema: { example: TEAM_EXAMPLE },
  })
  @ApiResponse({
    status: 404,
    description: 'Team not found, or Pokémon not found in PokéAPI',
    schema: {
  ...
  @ApiResponse({
    status: 409,
    description: 'Pokémon already in this team',
    schema: {
      example: { statusCode: 409, error: 'CONFLICT', message: 'Pokemon is already in the team' },
    },
  })
  @ApiResponse({
    status: 422,
    description: 'Team is full (max 5) or archived',
    schema: {
      example: {
        statusCode: 422,
        error: 'UNPROCESSABLE_ENTITY',
        message: 'Team is full (max 5 Pokémon)',
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      'Removes a Pokémon from the team by its local UUID (the `id` field from the team roster, not the PokéAPI ID). The slot becomes available again.',
  })
  @ApiParam({
    name: 'teamId',
    description: 'Team UUID',
  ...
  @ApiParam({
    name: 'pokemonId',
    description: 'TeamPokemon slot UUID (the `id` from the roster, not pokeapi_id)',
    example: 'c3d4e5f6-a7b8-9012-cdef-123456789012',
  })
  @ApiResponse({ status: 204, description: 'Pokémon removed from team (no body)' })
  @ApiResponse({
    status: 404,
    description: 'Team or Pokémon slot not found',
    schema: {
      example: { statusCode: 404, error: 'NOT_FOUND', message: 'Pokemon slot not found in team' },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      'Calculates cumulative type effectiveness (weaknesses ×2/×4, resistances ×0.5/×0.25, immunities ×0) for all Pokémon in the team. Type data is cached locally (TTL 7 days) and fetched from PokéAPI on miss. Multipliers are accumulated multiplicatively across all team members.',
  })
  @ApiParam({
    name: 'id',
    description: 'Team UUID',
  ...
  @ApiResponse({
    status: 200,
    description: 'Type effectiveness analysis',
    schema: { example: ANALYSIS_EXAMPLE },
  })
  @ApiResponse({
    status: 404,
    description: 'Team not found or empty',
    schema: {
      example: {
        statusCode: 404,
        error: 'NOT_FOUND',
